...
/datum/interaction/lewd/evaluate_user(mob/living/user, silent = TRUE)
	if(..(user, silent))
		...
		if(extreme)
			...
			if(cli)
				if(cli.prefs.extreme_verbs == "No")
					if(!silent)
						to_chat(user, "<span class = 'warning'>That's way too much for you.</span>")
					return FALSE

		if(require_ooc_consent)
			if(user.client && user.client.prefs.toggles & (user.client.prefs.lewd_verbs == "Yes"))
				return TRUE
		return FALSE
	return FALSE
...
/datum/interaction/lewd/evaluate_target(mob/living/user, mob/living/target, silent = TRUE)
	if(..(user, target, silent))
		...
		if(extreme)
			...
			if(cli)
				if(target.client.prefs.extreme_verbs == "No")
					if(!silent)
		...
		if(require_ooc_consent)
			if(target.client && target.client.prefs.toggles & (user.client.prefs.lewd_verbs == "Yes"))
				return TRUE
		return FALSE
	return FALSE
...
/mob/living/list_interaction_attributes(var/mob/living/LM)
	...
	if(client)
		...
		if(cli.prefs.extreme_verbs != "No")
			if(!ucli || (ucli.prefs.extreme_verbs != "No"))
