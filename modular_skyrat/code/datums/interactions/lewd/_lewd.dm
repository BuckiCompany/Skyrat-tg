		...
		if(peepee)
			switch(nintendo)
				if(REQUIRE_ANY)
					return TRUE
				if(REQUIRE_EXPOSED)
					if(peepee.visibility_preference != GENITAL_HIDDEN_BY_CLOTHES)
						return TRUE
					else
						return FALSE
...
/mob/living/proc/get_unconsenting(var/extreme = FALSE, var/list/ignored_mobs)
	...
	for(var/mob/M in range(7, src))
		if(M.client)
			...
			if(!(cli.prefs.toggles & (cli.prefs.lewd_verbs == "Yes"))) //Note: This probably could do with a specific preference
				nope += M
			else if(extreme && (cli.prefs.extreme_verbs == "No"))
				nope += M
		else
			nope += M
	return nope
